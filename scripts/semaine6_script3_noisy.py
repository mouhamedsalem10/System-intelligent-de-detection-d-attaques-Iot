...
print("=" * 60)
print("SEMAINE 6 — SCRIPT 3 : ÉVALUATION BRUITÉ ±20%")
print("Dataset : noisy20.csv")
print("=" * 60)

engine = EdgeInference(model_dir="models")

# Charger le dataset bruité
noisy_path = "data/noisy20.csv"
if not os.path.exists(noisy_path):
    print(f"❌ Fichier non trouvé : {noisy_path}")
    exit(1)

df     = pd.read_csv(noisy_path, sep=',')
y_true = df['label'].astype(int).values
types  = df['type'].astype(str).values \
         if 'type' in df.columns else None

print(f"Dataset bruité : {len(df)} flux")
...
print(f"""
MÉTRIQUES GLOBALES — Scénario Bruité ±20%
...
ax.set_ylabel('Réel')
ax.set_title(
    f'Scénario Bruité ±20% — F1={f1:.4f}\n'
    f'(Bruit gaussien multiplicatif sur 16 features)',
    fontsize=12
)
for i in range(2):
    for j in range(2):
...
plt.tight_layout()
plt.savefig('results/s6_noisy.png', dpi=150,
            bbox_inches='tight')
print("✅ results/s6_noisy.png")

with open("results/s6_metriques_noisy.json", "w") as f:
    json.dump({
        "scenario"  : "Bruité ±20%",
        "dataset"   : "noisy20.csv",
        "n_flux"    : int(len(df)),
...
print("✅ results/s6_metriques_noisy.json")
print(f"\n✅ SCRIPT 3 TERMINÉ — F1={f1:.4f}")